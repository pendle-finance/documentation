...
生息代币（IBT）通常会随着奖励和收益的累积而随时间升值，导致 IBT 与其会计资产之间的[汇率](./Glossary#汇率exchange-rate)持续上升。例如，1 个 sUSDe（IBT）可以随时间兑换更多 USDe。

然而，在收益转为负值的情况下，IBT 的价值可能反而下降。这会影响 PT 和 YT 的表现，尤其是在汇率下跌时。

## 什么是水位线汇率？

[水位线汇率（Watermark Rate）](./Glossary#水位线汇率watermark-rate) 是 IBT 与其会计资产之间汇率的历史最高记录值。在长期负收益期间，汇率可能跌破这一水位。
