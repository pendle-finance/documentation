...
所有奖励渠道每周的最大发放总量为 90,000 枚 PENDLE。其中每个资金池在绩效奖励渠道和限价单渠道中，每周最多可分别获得价值 $3,000 和 $1,250 的 PENDLE 奖励。若某周内有未分配的 PENDLE，将退回协议国库，不结转至下一周预算。
...
资金池根据其手续费表现获得 PENDLE 激励。激励率由资金池的加权 TVL 层级决定，使较小的池子在表现出色时能够获得更多奖励。
...
对于处于加权 TVL 区间内的资金池，采用线性插值来确定激励率。上线 28 天后，所有资金池将回归至 0.5 的基准激励率。
...
资金池根据其总 TVL 与近期交易量获得限价单（LO）奖励，每个池子每周上限为 1,250 美元。激励将以时间加权和名义价值加权的方式分配给当前 IY 4% 范围内的限价单。

### 目标深度（Target Depth）

每个池子以其总 TVL 的 10% 作为订单簿深度的基准目标，并可根据其 4 日近期调整交易量（4D-RASW）增长至总 TVL 的 15%。
