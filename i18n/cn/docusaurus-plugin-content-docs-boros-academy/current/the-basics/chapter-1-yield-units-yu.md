...
每个 YU 都有一个到期日。到期后，YU 将不再产生任何收益，所有支付义务均已结清。
...
YU 本质上代表资产的未来收益。因此，YU 的合理价值是你在到期前将收取的总收益之和。

<div align="center" data-full-width="true"><figure><img src="/boros-academy/imgs/image (18).png" alt="" /><figcaption></figcaption></figure></div>

以持有 YU-ETHUSDT-Binance 多头头寸的交易者为例，每次结算后都会收取收益。因此，假设隐含 APR 不变，每次结算后 YU 的价值也会随之下降——因为需要计入的未来收益已经减少。你可以在[第 4 章](chapter-4-settlement)中了解更多关于结算的内容。

到期时，由于不再有任何收益义务，整个头寸将全部结清。届时，YU 的价值为零。
