...
`MarketState` is the low-level AMM state struct returned by `getMarketState` and `readMarketState`. See [PendleMarket](../PendleMarket/PendleMarket.md) for field-level documentation.
