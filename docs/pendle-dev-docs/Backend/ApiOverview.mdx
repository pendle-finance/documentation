...
Each user (IP) has a rate limit of 200 CU per minute, and 200,000 CU per week.
...
| `X-RateLimit-Limit` | Maximum CU per minute (e.g., 200 for free tier) |
...
You may occasionally notice `X-Ratelimit-Limit: 200` in responses even when you have a higher rate limit plan. 
...
The `X-Ratelimit-Limit: 200` value in this case is simply a default header from the cached response and does not reflect your actual rate limit.
...
A: Obtain API keys from aggregator partners (KyberSwap, OKX, ParaSwap) and pass them in request headers. This reduces that aggregator's CU cost to 0. See [Reducing Aggregator Costs](./HostedSdk.mdx#reduce-aggregator-computing-units).
