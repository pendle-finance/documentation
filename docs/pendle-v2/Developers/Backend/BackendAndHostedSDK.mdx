...
| `odos`      | `ODOS_API_KEY`     |             ---   |
| `okx`       | `OKX_ACCESS_KEY, OKX_ACCESS_SECRET, OKX_PASSPHRASE`      | Need all three to return a valid response |
...
Computing unit for the custom aggregator is deduce from the public one. For example if you are using public Odos API key, the computing unit cost will be 5 (base cost) + 15 (odos cost) = 20 computing units, but when you use your own Odos API key, the computing unit cost will be 5 (base cost) + 0 (odos cost with custom API key) = 5 computing units.
