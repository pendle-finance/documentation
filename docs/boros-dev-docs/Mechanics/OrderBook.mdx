...
Each order in Boros has a unique order Id, which is a 64-bit packed value containing order metadata:
...
**Limit Order Rate Bounds**:

Limit orders have additional rate constraints controlled by four parameters in the market config:
