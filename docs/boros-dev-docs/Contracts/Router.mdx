...
The ConditionalModule enables orders with off-chain trigger conditions.
...
:::note
Conditional orders are not submitted directly to the contract by users. The agent signs the `ConditionalOrder`, and a permissioned validator submits the matching execution signature once the off-chain condition is met.

Integrators reach this through the backend rather than the contract — see [Stop Orders](/boros-dev/Backend/stop-orders).
:::
