...
:::note
Conditional orders are not placed directly by users. The agent signs the `ConditionalOrder`, and a permissioned validator submits the matching execution signature once the off-chain condition is met.
:::
