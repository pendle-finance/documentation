...
Pendle Boros is an on-chain platform for trading interest rate swaps. The protocol enables users to take long or short positions on variable interest rates with leverage through a hybrid system combining a central limit order book and AMM. At launch, Boros only supports trading of funding rates, though Boros is designed to allow trading of any type of interest rate that has an index oracle.
...
- **SDK Integration** - Use the Boros SDK for simplified integration through agent-based calls. The SDK handles transaction management, signature generation, and provides a high-level API for common trading operations.
  - [Backend Integration Overview](./Backend/0.%20overview.mdx)
...
- **REST API** - Query market data, order book snapshots, historical trades, and account information
  ...
  - [API Documentation](./Backend/3.%20api.mdx)

- **WebSocket** - Real-time data streaming for order book updates, trades, and market events
  - [WebSocket Documentation](./Backend/4.%20websocket.mdx)

- **Historical Data** - Monthly exports of market data, trades, OHLCV candles, order book snapshots, and settlement data in NDJSON format
  - [Historical Data Documentation](./Backend/7.%20historical-data.mdx)
