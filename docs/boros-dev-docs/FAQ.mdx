...
Up to **10 markets** per market account. If you've hit the limit, exit markets you're no longer trading. See [Best Practices](./Backend/5.%20best-practices.mdx) for why exiting unused markets also saves gas.
...
**Single orders** (`placeSingleOrder` on-chain) can match with the AMM for potentially better execution but cost more gas if placing multiple.

**Bulk orders** are batched into one transaction — lower gas, atomic execution, but they go directly to the order book and do **not** match with the AMM.

Use bulk orders when placing or adjusting multiple orders at once. See [Best Practices](./Backend/5.%20best-practices.mdx#bulk-order-placement).
...
Agent approvals have an expiry time. Check it with `GET /agents/expiry-time` and re-approve if needed using `GET /calldata/approve-agent`.
