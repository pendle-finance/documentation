...
**Step 3** returns an array of calldata objects. For bulk orders, multiple operations are batched into a single transaction.

**Step 5** sends the signed calldata to the Send Txs Bot, which submits the on-chain transaction. The response includes a transaction status and hash.
...
| `GET /accounts/transactions` | Trade history on a market |
