...
### Collaterals
Query collateral summaries for a user's account, including cross-margin and isolated positions with PnL, margins, and balances.

| Endpoint | Description |
|----------|-------------|
| `GET /collaterals/summary/` | All collateral summaries for an account. Returns per-token breakdown of cross/isolated positions, net balances, margins, PnL (trade + settlement + unrealised), and withdrawal info. Includes `syncStatus`. |
| `GET /collaterals/summary/single` | Single collateral summary by `tokenId`. Same response shape as above but for one token. Includes `syncStatus`. |

Both endpoints require `userAddress` and `accountId` query parameters. The `/single` endpoint additionally requires `tokenId`.
