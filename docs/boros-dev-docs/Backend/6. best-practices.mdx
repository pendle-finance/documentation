...
- [Place Orders API](https://api-boros.pendle.finance/apis/docs)
...
:::caution Market entry on the new mount
...
The Send Txs Bot service uses a legacy format:
...
| Market not entered | Trying to trade on a market you haven't entered | Call `POST /calldata-builder/agent/enter-markets` first, then place |
...
### Recovering from Network Errors (Trace + Nonce)

When a Send Txs Bot submission times out or returns a network error, you can't tell whether the bot received the request. Use the [trace endpoint](https://api-boros.pendle.finance/send-txs-bot/docs#/Dedicated%20Agent/DedicatedAgentController_trace) (`POST /send-txs-bot/v1/agent/dedicated/trace`) to disambiguate before retrying.

**Recommended pattern:**
1. After a network error/timeout, poll the trace endpoint with the `agent` address and the `nonce` of the calldata you submitted.
2. If the request can be traced, the bot received it — wait for the on-chain result instead of retrying.
3. If it cannot be traced after **2–3 seconds**, assume the bot never received it and retry safely.

Trace records are retained for **24 hours**.

:::tip Why retry is safe
The Send Txs Bot rejects any calldata signed with a `nonce` ≤ your largest previously-seen nonce — and this guarantee includes nonces from calldata that failed simulation. So if the original submission had actually succeeded, the retry would be rejected by the backend rather than re-executed. You cannot double-submit by retrying.
:::
...
| Order book depth | `GET /open-api/v2/markets/order-books?marketId={marketId}` | < 1 second |
| AMM state | `GET /core/amm/{ammId}` | < 1 second |
| Market list / overview | `GET /open-api/v1/markets` | May have several seconds of delay |

:::tip
For market making or any use case requiring live mid/mark rates, always use the **single market endpoint** (`GET /open-api/v1/markets/{marketId}`) rather than the bulk markets list. The bulk endpoint may have a delay of several seconds on sensitive indicators.
:::
...
Don't poll REST endpoints for real-time data. Use [WebSocket](./5.%20websocket.mdx) for:
...
| Use `bulk-direct-call` whenever you have ≥1 calldata to submit | Sequential ordered execution; supports `requireSuccess` atomicity |
| Use `requireSuccess=false` for independent orders | Lets each order succeed/fail independently (market making) |
| Call `enter-markets` explicitly before trading a new market | New Open API mount does not auto-enter; place calls revert otherwise |
| Exit unused/expired markets via `exit-markets` (max 10 per account) | Reduce gas overhead, free up slots |
