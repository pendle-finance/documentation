...
The SDK ships with the production backend pre-wired (`https://api-boros.pendle.finance/apis`). All endpoints — markets, accounts, calldata builders, and send-txs — go through this single host; no setup is needed.
...
| Default Open API base URL | an internal host, not publicly reachable | `https://api-boros.pendle.finance/apis` |
...
```ts
// Open API (renamed)
BorosBackend.setCoreBackendUrl(url);
//                       ↓
setOpenApiBackendUrl(url);
...
The `env`-driven environment auto-switch from the old package is gone. The public package always targets production, baked in as the default — calling the setter is not required:
