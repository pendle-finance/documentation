...
`StopOrders` wraps take-profit / stop-loss: `list`, `get`, `prepare`, `place`, `cancel`, where `place()` runs prepare → agent-sign → submit. It needs an [API signing key](./7.%20authentication.mdx) installed via `setOpenApiSigningKey()`, and 0.2.0 or later. See [Stop Orders](./13.%20stop-orders.mdx).
