...
| | |
|---|---|
| Package | [`@pendle/boros-sdk-public`](https://www.npmjs.com/package/@pendle/boros-sdk-public) on npm |
| Source | [github.com/pendle-finance/boros-sdk-public](https://github.com/pendle-finance/boros-sdk-public) |
| Runnable examples | [github.com/pendle-finance/boros-api-examples](https://github.com/pendle-finance/boros-api-examples) |
...
`viem` is a direct dependency pinned to an exact version, not a peer dependency. Install a range that covers the pinned one, or your package manager resolves a second copy and the `WalletClient` you construct stops matching the type the SDK declares. The math helpers (`FixedX18`, `estimateTickForRate`, `getRateAtTick`) live in a separate package — install it when you need tick/rate conversion:
...
Take-profit / stop-loss orders live behind a second credential — an [API signing key](./7.%20authentication.mdx). Install it once and the SDK signs every open-api request for you:

```ts
import {
  ApiSigningKey, setOpenApiSigningKey, setInternalAgent,
...
setOpenApiSigningKey(new ApiSigningKey({
  ...
  privateKey: process.env.BOROS_API_KEY_PEM!, // PKCS#8 PEM, shown once at creation
}));

const stopOrders = new StopOrders();          // no accountId — it comes from the key

// Reads need only the signing key.
const live = await stopOrders.list({ isActive: true });

// Writes also need an approved agent.
...
Requires **0.2.0** or later. `setOpenApiSigningKey()` covers the data endpoints but **not** the `/v1/api-keys` tree, which takes an EIP-712 root signature instead. Full endpoint reference: [Stop Orders](./13.%20stop-orders.mdx).

## Escape hatch — raw open-api access

The wrapped methods cover common trading flows. For everything else (leaderboard, OHLCV, settlement events, indicators), drop down to the codegen client:
