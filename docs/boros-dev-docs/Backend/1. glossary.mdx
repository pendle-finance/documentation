...
| **Stop Order** *(= Conditional Order)* | Two names for one thing: an order with an off-chain trigger condition, executed by a permissioned validator once the market APR crosses `stopApr`. **Conditional order** is the contract's name for the mechanism ([Router `ConditionalModule`](/boros-dev/Contracts/Router#conditional-orders)); **stop order** is the product name for the only two kinds that exist — `orderType` `2` take-profit, `3` stop-loss. Requires an API signing key. See [Stop Orders](/boros-dev/Backend/stop-orders). | `GET/POST /v1/stop-orders/*`, Router `ConditionalModule` (on-chain) |
