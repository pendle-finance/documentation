...
| **Tick** | Integer price level representing an interest rate. Formula: `rate = 1.00005^(tick × tickStep) - 1`. Higher tick = higher rate. The tick step is market-specific and controls rate granularity. Use SDK helpers to convert between human-readable APR and tick values. | All order-related endpoints |
...
| **Conditional Order** | On-chain order with an off-chain trigger condition, executed by a permissioned validator when conditions are met. Exposed to integrators as [Stop Orders](/boros-dev/Backend/stop-orders); the contract side is [Conditional Orders](/boros-dev/Contracts/Router#conditional-orders). | `GET/POST /v1/stop-orders/*`, Router `ConditionalModule` (on-chain) |
| **Stop Order** | A take-profit or stop-loss: a conditional order triggered by the market APR crossing `stopApr`. `orderType` `2` = take-profit, `3` = stop-loss. Requires an API signing key. See [Stop Orders](/boros-dev/Backend/stop-orders). | `GET/POST /v1/stop-orders/*` |
| **API Signing Key** | Ed25519 keypair proving which **root** is calling. Every request carries a self-signed JWT in `x-pendle-auth`; nothing static goes on the wire. Proves identity only — it cannot move funds without an agent signature. See [Authentication](/boros-dev/Backend/authentication). | `POST /v1/api-keys` |
