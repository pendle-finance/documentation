...
| Term | Description | Where it appears |
|------|-------------|-----------------|
| **Computing Unit (CU)** | Per-endpoint cost of a request, advertised via the `x-computing-unit` OpenAPI extension and returned as the `x-computing-unit` response header. A value like `1+` means **dynamic cost** — the endpoint charges a minimum for an empty request and scales with work performed (e.g. page size, number of indicators). See [Computing Units](./9.%20computing-units.mdx). | All endpoints (header) |
| **Indicator** | Compact letter codes for time-series market data: `u` (underlying APR), `fp` (future premium), `fgi` (fear & greed index), `ap` (asset price), `udma:<periods>` (X-day UFR moving averages). See [Indicators](./8.%20indicators.mdx). | `GET /v1/indicators`, `GET /v1/indicators/export` |
...
- [Indicators](./8.%20indicators.mdx) — Indicator letter codes (`u`, `fp`, `fgi`, `ap`, `udma`)
- [Computing Units](./9.%20computing-units.mdx) — How endpoint cost is computed and reported
