...
| **Conditional Order** | On-chain order with an off-chain trigger condition, executed by a permissioned validator when conditions are met. See [Conditional Orders](/boros-dev/Contracts/Router#conditional-orders). | Router `ConditionalModule` (on-chain) |
