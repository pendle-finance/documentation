...
| Value | Name | Description |
|-------|------|-------------|
| `0` | `GOOD_TIL_CANCELLED` | Standard limit order that remains on the order book until fully filled or manually canceled |
| `1` | `IMMEDIATE_OR_CANCEL` | Executes against available liquidity immediately, cancels any unfilled portion |
| `2` | `FILL_OR_KILL` | Must fill entire order size immediately or the transaction reverts |
| `3` | `ALO` | Add Liquidity Only order that adds liquidity to the book, transaction reverts if it would match existing orders |
| `4` | `SOFT_ALO` | Soft Add Liquidity Only order, similar to ALO but skips matching without reverting the transaction |
