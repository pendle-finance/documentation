...
Requests that carry no credential are metered **per IP**, against the shared
anonymous budget. To be metered under your own plan instead, identify yourself
with an API signing key.
...
3. Choose a plan, then create an API signing key on the `/api-keys` page

Keys can also be minted from a script — see [Authentication](./7.%20authentication.mdx).
The private key is shown **once**; store it before closing the dialog.

### Using your key

Sign a short-lived JWT with the key and send it as `x-pendle-auth` on every
request. The gateway resolves your root wallet from it and meters you at your
plan's limits.

```bash
curl -i -H "x-pendle-auth: $TOKEN" \
  https://api-boros.pendle.finance/apis/v1/markets
```

Your plan is attached to your **root wallet address**, so one key is enough for
both purposes: it authenticates you on the endpoints that require it, and it
identifies you for metering on all the others. See
[Authentication](./7.%20authentication.mdx) for the JWT claims and a minting
helper.
