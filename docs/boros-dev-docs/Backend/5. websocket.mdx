...
:::caution WebSocket is in beta
The WebSocket service is currently a **beta experiment**. Channels, event names and payloads may change, and delivery is not guaranteed — events can be delayed or dropped, especially around reconnects.

Do not rely on it as the sole source of truth for your account state. For now, fetch your current positions from the REST API — either on a cron/polling schedule or after each WebSocket notification:

- [`GET /accounts/active-positions`](./4.%20api.mdx#active-positions) — all active positions for a user
- [`POST /accounts/market-acc-infos`](./4.%20api.mdx#market-acc-infos) — positions, margins and balances per market account
:::
...
const socket = io('wss://api-boros.pendle.finance/pendle-dapp-v3', {
...
class PendleWebSocket {
  ...
  constructor() {
    this.socket = io('wss://api-boros.pendle.finance/pendle-dapp-v3', {
      path: '/socket/socket.io',
      reconnectionAttempts: 5,
      transports: ['websocket']
    });
...
You can use `AccountLib.pack()` from `@pendle/boros-sdk-public` to construct this:

```typescript
import { AccountLib } from '@pendle/boros-sdk-public';
...
class AccountTracker {
  ...
  private async refreshAccountData() {
    try {
      ...
      const { data } = await axios.post('https://api-boros.pendle.finance/apis/v1/accounts/market-acc-infos', {
        marketAccs: [
          /* your marketAcc */
        ],
      });
