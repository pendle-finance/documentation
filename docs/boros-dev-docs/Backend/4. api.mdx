...
| **Send Txs Bot** | `https://api-boros.pendle.finance/send-txs-bot/v2/` (programmatic) or `/v3/` (browser session) | Submitting signed transactions on-chain. |
...
Most endpoints on this page are public — market data and calldata generation need
no credential, and actions that move funds carry their authorization as an EIP-712
signature in the request body rather than in a header.

Two cases differ:

- **[Stop Orders](./13.%20stop-orders.mdx)** require an API signing key, sent as a
  short-lived JWT in `x-pendle-auth`.
- **Rate limits** are metered per IP unless you send that same header, in which
  case you are metered under your own plan.

See [Authentication](./7.%20authentication.mdx) for both.
...
**Interactive docs:** [https://api-boros.pendle.finance/send-txs-bot/docs](https://api-boros.pendle.finance/send-txs-bot/docs)
...
| `POST /send-txs-bot/v1/agent/dedicated/trace` | Trace a submitted request by `agent` address + calldata `nonce` to check whether the Send Txs Bot received it. Useful for resolving ambiguity after network errors/timeouts. Records are retained for **24 hours**. See [Recovering from Network Errors](./6.%20best-practices.mdx#recovering-from-network-errors-trace--nonce) for the recommended retry pattern. |
...
The Send Txs Bot service uses the legacy format:
